...
// netlify/functions/_lib/prices.js
var require_prices = __commonJS({
  "netlify/functions/_lib/prices.js"(exports2, module2) {
    var PRICES2 = {
      "Elegant Pearl": 27,
      "Lavender Bloom": 27,
      "Lilac Silver": 27,
      ...
      "Kundan": 30,
      "Sunrise Bliss": 30
      // 'Arm Cuff' is still a placeholder product in the catalog — add its
      // real price here once it's a real listing, then it will be purchasable.
    };
    module2.exports = { PRICES: PRICES2 };
  }
});

// netlify/functions/create-checkout-session.js
var stripe = require_stripe_cjs_node()(process.env.STRIPE_SECRET_KEY);
var { PRICES } = require_prices();
exports.handler = async (event) => {
  ...
  try {
    const line_items = items.map((item) => {
      ...
      if (unitPrice === void 0) {
        throw new Error(`"${item.name}" is not available for purchase right now.`);
      }
      ...
      return {
        price_data: {
          currency: "usd",
          product_data: {
            name: `${item.name} \u2014 Size ${item.size || "S"}`
          },
          // Stripe wants the smallest currency unit (cents), never a float dollar amount.
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items,
      metadata: {
        cart: JSON.stringify(cartSummary).slice(0, 500)
        // Stripe metadata values cap at 500 chars
      },
      // Collects full name + billing address on the Stripe-hosted page.
      billing_address_collection: "required",
      // Collects a phone number on the Stripe-hosted page.
      phone_number_collection: { enabled: true },
      // Collects a shipping address. Add more countries as you expand where you ship.
    ...
    return {
      statusCode: 400,
      body: JSON.stringify({ error: err.message || "Could not start checkout." })
    };
  }
};
